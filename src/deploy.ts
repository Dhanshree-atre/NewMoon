/**
 * Deploy PrivateAirdrop to a Midnight network (undeployed by default; use
 * --network preview|preprod for public networks).
 *
 * Non-interactive: scaffold → npm run setup runs straight through.
 *
 * Campaign parameters (all optional; sane defaults for the local devnet):
 *   --name        campaign display name            (default "PrivateAirdrop Demo")
 *   --campaign-id 32-byte hex campaign id          (default = sha256 of name)
 *   --reward      reward per claim (u8)            (default 100)
 *   --max-claims  maximum number of claims (u8)    (default 16)
 *
 * The admin secret key is derived deterministically from the wallet seed, so
 * the same wallet can close the campaign later without storing any secret key.
 */
...
import { emptyAirdropPrivateState, deriveAdminSecretKey, fromHex } from './witnesses';
...
const campaignName = argValue('--name', 'PrivateAirdrop Demo Campaign');
let campaignIdHex = argValue('--campaign-id', '');
const rewardPerClaim = BigInt(argValue('--reward', '100'));
const maxClaims = BigInt(argValue('--max-claims', '16'));

if (!campaignIdHex) {
  campaignIdHex = await sha256Hex(campaignName);
}
const campaignId = fromHex(campaignIdHex);
if (campaignId.length !== 32) {
  console.error(`\n❌ --campaign-id must be 32 bytes (64 hex chars), got ${campaignId.length * 2}.\n`);
  process.exit(1);
}
...
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'private-airdrop');
...
const PrivateAirdrop = await import(pathToFileURL(contractPath).href);

const compiledContract = CompiledContract.make('private-airdrop', PrivateAirdrop.Contract).pipe(
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║  Deploy PrivateAirdrop campaign to ' + network);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');

  console.log(`  Campaign:      ${campaignName}`);
  console.log(`  Campaign ID:   0x${toHex(campaignId)}`);
  console.log(`  Reward/claim:  ${rewardPerClaim}`);
  ...
  // Derive the admin secret key from the wallet seed. This value is private:
  // it is used only inside the constructor proof and never printed.
  const adminSk = await deriveAdminSecretKey(Buffer.from(seed, 'hex'));
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      deployed = await deployContract(providers, {
        ...
        args: [campaignName, campaignId, rewardPerClaim, maxClaims],
        privateStateId: PRIVATE_STATE_ID,
        initialPrivateState: emptyAirdropPrivateState(adminSk),
      });
      break;
    } catch (err: any) {
